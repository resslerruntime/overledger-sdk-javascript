...
describe('Dlt/Ripple', () => {
  ...
  test('Cannot sign a ripple transaction without providing the message parameter', () => {
    expect(() => overledger.dlts.ripple.sign({any: '...', dlt: DltNameOptions.xrp, type: TransactionTypeOptions.accounts, subType: TransactionSubTypeOptions.valueTransfer})).toThrow('Error parameter: message. Error is: All transactions must have a message field. If no message is required, assign message to the empty string, i.e. message: ""');
  });
  ...
  test('Cannot sign a ripple transaction without providing the fromAddress parameter', () => {
    expect(() => overledger.dlts.ripple.sign({any: '...', dlt: DltNameOptions.xrp, type: TransactionTypeOptions.accounts, subType: TransactionSubTypeOptions.valueTransfer, message: ""})).toThrow('Error parameter: fromAddress. Error is: All transactions for accounts distributed ledgers must have the fromAddress field');
  });
  test('Cannot sign a ripple transaction without providing the toAddress parameter', () => {
    expect(() => overledger.dlts.ripple.sign({any: '...', dlt: DltNameOptions.xrp, type: TransactionTypeOptions.accounts, subType: TransactionSubTypeOptions.valueTransfer, message: "", fromAddress: "abcdefg"})).toThrow('Error parameter: toAddress. Error is: All transactions for accounts distributed ledgers must have the toAddress field. If you do not want to set a toAddress (maybe you are creating an on-chain smart contract?), assign toAddress to the empty string, i.e. toAddress = ""');
  });
  test('Cannot sign a ripple transaction without providing the sequence parameter', () => {
    expect(() => overledger.dlts.ripple.sign({any: '...', dlt: DltNameOptions.xrp, type: TransactionTypeOptions.accounts, subType: TransactionSubTypeOptions.valueTransfer, message: "", fromAddress: "abcdefg", toAddress: "pqrst"})).toThrow('Error parameter: sequence. Error is: All transactions for accounts distributed ledgers must have the sequence field as a number');
  });
  ...
  test('Can sign a ripple transaction', async () => {
    signedTransaction = await overledger.dlts.ripple.sign({any: '...', dlt: DltNameOptions.xrp, type: TransactionTypeOptions.accounts, subType: TransactionSubTypeOptions.valueTransfer, message: "", fromAddress: "rndaCtYjxKq3vBTA3ER1SAPSgvQRMXQZnz", toAddress: "rLEBHTbZBeSaY4ghcjLQMYvFEgKFyoRXbp", sequence: 1, amount: 1, extraFields: {feePrice: "1", maxLedgerVersion: "12345"}});
